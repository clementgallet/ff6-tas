...
-- 7. Criticals and Desperation attacks are approximately predictedpredicted
...
function display_battle()
	local sel_side, sel_i = memory2.WRAM:byte( 0x7a9e), memory2.WRAM:byte( 0x7a9f)
	local sel_enemy, sel_party = memory2.WRAM:byte( 0x7b4e),memory2.WRAM:byte( 0x7b4d)
	local sel_multi = memory2.WRAM:byte( 0x7b4f)
	-- loop through present monsters
	for i = 0, 5 do
		-- a few shortcuts
		local j, k = 2*i, 2*(4+i)
		local state = memory2.WRAM:word( 0x3aa0+k)
		if state ~= 0 and bit.test(state,15) ~= 1 then
		-- get coordinates (we should know the size of the monster too, but we don't)
		local x = memory2.WRAM:byte( 0x8092+1+j)
		...
		-- I can't be bothered
		local xsize = memory2.WRAM:byte( 0x804a+1+j)
		if xsize == 0 then xsize = 0x20 end
		local y = memory2.WRAM:byte( 0x809e+1+j)
		-- other interesting information
		local id   = memory2.WRAM:word( 0x3388+j)
		local xp   = memory2.WRAM:word( 0x3d8c+j)
		local gold = memory2.WRAM:word( 0x3da0+j)
		local hp   = memory2.WRAM:word( 0x3bf4+k)
		local mp   = memory2.WRAM:word( 0x3c08+k)
		local mhp  = memory2.WRAM:word( 0x3c1c+k)
		local mmp  = memory2.WRAM:word( 0x3c30+k)
		local level= memory2.WRAM:byte( 0x3b18+k)
		local speed= memory2.WRAM:byte( 0x3b19+k)
		local gauge= memory2.WRAM:word( 0x3218+k)
		-- elements
		local absorb  =  memory2.WRAM:byte( 0x3bcc+k)
		local nullify =  memory2.WRAM:byte( 0x3bcd+k)
		local resist  =  memory2.WRAM:byte( 0x3be1+k)
		local weak    =  memory2.WRAM:byte( 0x3be0+k)
		local stealn  =  memory2.WRAM:byte( 0x3311+j)
		local stealr  =  memory2.WRAM:byte( 0x3310+j)
		local dropn   =  memory2.ROM:byte( 0x0f3000+4*id+3)
		local dropr   =  memory2.ROM:byte( 0x0f3000+4*id+2)
		...
		-- gui.text(x+18, by+8, ""..gauge)

		local active_slot = memory2.WRAM:byte( 0x0201)
		local active_player = memory2.WRAM:byte( 0x3ed8+active_slot*2)
		local menu_pos = memory2.WRAM:byte( 0x88df+active_slot)
		...
		if gau_info then
			local id_byte, id_bit = bit.quotent(id,8), id % 8
			if bit.test(memory2.WRAM:byte( 0x1d2c+id_byte),id_bit) == 0 then
				-- monster not known!
				gui.text(x-8,y,"?")
			end
		end
		...
		if (sel_side == 0 or sel_side == 2) and sel_i == i and
			...
			if DISPLAY_CRITICALS then
				be = memory2.WRAM:byte( 0x00be)
				be = (be + 0x31) % 0x100
				rng = memory2.ROM:byte( 0x00fd00+be)
				if rng < 0x08 then
					gui.text(10,offset,string.format("Critical (%X)",be))
					offset = offset+10
				else
					gui.text(10,offset,string.format("(%X)",be))
					offset = offset+10
				end
			end
			if DISPLAY_DA then
				battle_counter = memory2.WRAM:word( 0x3a3e)
				gui.text(10,offset,string.format("Battle Counter: %X",battle_counter))
				offset = offset+10
				be = memory2.WRAM:byte( 0x00be)
				be = (be + 0x32) % 0x100
				rng = memory2.ROM:byte( 0x00fd00+be)
				if bit.band(rng, 0x0F) == 0 then
					gui.text(10,offset,string.format("DA (%X)",be))
					offset = offset+10
				else
					gui.text(10,offset,string.format("(%X)",be))
					offset = offset+10
			...
			if relm_info then
				-- Sketch
				local sketch1 = memory2.ROM:byte( 0x0f4300+2*id)
				local sketch2 = memory2.ROM:byte( 0x0f4300+2*id+1)
				-- control
				local control, controln = {}, {}
				for ctrl_i = 0, 3 do
					control[ctrl_i] = memory2.ROM:byte( 0x0f3d00+4*id+ctrl_i)
				end
	...
	if memory2.WRAM:byte( 0x7bd4) == 0x24 then
		local x = memory2.WRAM:byte( 0x88ff)
		local y = memory2.WRAM:byte( 0x8903)
		local scroll = memory2.WRAM:byte( 0x88fb)
		local i = 2*(y+scroll)+x
		local id = memory2.WRAM:byte( 0x257e + i)
		if id ~= 0xFF then
			-- What rages does this monster have?
			local rage1 = memory2.ROM:byte( 0x0f4600+2*id)
			local rage2 = memory2.ROM:byte( 0x0f4600+2*id+1)
			-- Special attack description
			local specialn = get_special(id)
...
display_treasures = function()
	...
	local sx     = memory2.WRAM:word( 0x8267)
	local sy     = memory2.WRAM:word( 0x8269)
	if sx >= 0x8000 then sx = sx-0x10000 end
	if sy >= 0x8000 then sy = sy-0x10000 end
	-- prepare to loop through treasures
	local area   = memory2.WRAM:word( 0x0082)
	local table  = 0x2d8634
	local i      = memory2.ROM:word( 0x2d82f4+2*area)
	local to     = memory2.ROM:word( 0x2d82f4+2*(area+1))
	-- loop through the treasures
	while i ~= to do
		-- read from the treasure information array
		local xi     = memory2.ROM:byte( table+i)
		local yi     = memory2.ROM:byte( table+i+1)
		local flagi  = memory2.ROM:word( table+i+2)
		local item   = memory2.ROM:byte( table+i+4)
		local name   = getitemname(item)
		-- Has the box been taken? To find out, we need to
		-- do some bit manipulation.
		local byte_index = bit.quotent(flagi,8)
		local bit_index = flagi % 8
		if byte_index >= 0x40 then
			byte_index = byte_index - 0x40*bit.quotent(byte_index,0x40)
		end
		local flag = memory2.WRAM:byte( 0x1e40+byte_index)
		flag = flag % (bit.lshift(0x1, bit_index+1))
		flag = bit.quotent(flag,bit.lshift(0x1, bit_index))
		...
		gui.rectangle(rx, ry, 0x10, 0x10, 1, color)
		gui.text(rx, ry-5, name)

		i = i + 5
	end
	table = 0x1fbb00
	i     = memory2.ROM:word( table+2*area)
	to    = memory2.ROM:word( table+2*(area+1))
	-- loop through the transitions
	while i ~= to do
		-- read from the treasure information array
		local xi     = memory2.ROM:byte( table+i)
		local yi     = memory2.ROM:byte( table+i+1)
		local hmm    = memory2.ROM:word( table+i+2)
		local xti    = memory2.ROM:byte( table+i+4)
		local yti    = memory2.ROM:byte( table+i+5)
	...
	while i ~= to do
		...
		local xi     = memory2.ROM:byte( table+i)
		local yi     = memory2.ROM:byte( table+i+1)
		local dxi    = memory2.ROM:byte( table+i+2)
		local dyi    = 0
		local hmm    = memory2.ROM:word( table+i+3)
		local xti    = memory2.ROM:byte( table+i+5)
		local yti    = memory2.ROM:byte( table+i+6)
		local area2 = hmm % 0x200
		...
		-- relative coordinates
		local rx, ry = x-sx, y-sy
		gui.rectangle(rx, ry, dx+0x10, dy+0x10, 1, color)
	...
	while i ~= to do
		...
		local xi     = memory2.ROM:byte( table+i)
		local yi     = memory2.ROM:byte( table+i+1)
		local react1 = memory2.ROM:word( table+i+2)
		local react2 = memory2.ROM:byte( table+i+4)
...
function display_encounters()
	...
	for id = 0, 0xF do
		local tmp = memory2.WRAM:byte( 0x1850+id)
		if tmp % 8 == memory2.WRAM:byte( 0x1a6d) then
			...
			for i = 0, 5 do
				local item = memory2.WRAM:byte( 0x1600+x+i)
				if item ~= 0xFF then
					-- item actually equipped
					field_effects = bit.bor(field_effects,memory2.ROM:byte( 0x185005+item*0x1e))
					status_effects = bit.bor(status_effects,memory2.ROM:byte( 0x18500a+item*0x1e))
				end
			end
			
			-- Gather the speed of the corresponding character
			slot = bit.lrshift(bit.band(tmp, 0x18), 3)
			slot_speed[slot+1] = memory2.WRAM:byte( 0x161B+0x25*id)
	...
	if mode == MODE_MAP then
		local terrain = memory2.WRAM:byte( 0x11f9) % 8
		local at_22 = memory2.ROM:byte( 0x00c1c1+terrain) -- lave tall, men hva betyr de?
		local a = memory2.ROM:byte( 0x00c1c9+terrain) -- denne arrayen fÃ¸lger rett etter. samme stÃ¸rrelsesorden.
		-- b og c er faktisk x og y-posisjonen pÃ¥ kartet til *forrige encounter*!
		local last_x = memory2.WRAM:byte( 0x1f60)
		local last_y = memory2.WRAM:byte( 0x1f61)
		local zone_x, zone_y = bit.quotent(last_x,0x20),bit.quotent(last_y,0x20)
		local zone = memory2.WRAM:byte( 0x1f64)*0x40+zone_y*8+zone_x

		-- a mÃ¥ inneholde en mapping fra terreng til offsets i fÃ¸lgende array.
		-- merkelig at slikt trengs.
		pack_index = memory2.ROM:byte( 0x0f5400+zone*4+a)
		-- den hÃ¸ye byten her angir kanskje om det er world of balance eller ruin eller noe annet?
		local encounter_rate_index = bit.quotent(memory2.ROM:byte( 0x0f5800+zone), bit.lshift(0x1,2*at_22)) % 4
		...
		encounter_rate = memory2.ROM:word( 0x00c1d1+x)
	else
		local area = memory2.WRAM:word( 0x0082)
		local ax,ay = bit.quotent(area,4), area % 4
		local a = (bit.quotent(memory2.ROM:byte( 0x0f5880+ax), bit.lshift(0x1,2*ay))) % 4
		local x = ((memory2.WRAM:byte( 0x11df) % 4) * 4 + a) * 2
		encounter_rate = memory2.ROM:word( 0x00c1f1+x)
		if bit.test(memory2.WRAM:byte( 0x0525),7) == 0 then
			encounter_rate = 0
		end
		pack_index = memory2.ROM:byte( 0x0f5600+area)
	end

	if encounter_rate ~= 0 then
	
		-- Compute how many steps until the next fight
		local enc1 = memory2.WRAM:word( 0x1f6e)
		local enc2 = memory2.WRAM:byte( 0x1fa1)
		local enc3 = memory2.WRAM:byte( 0x1fa4)
		local count = -1

		for i = 0, 250 do
			enc1 = enc1 + encounter_rate
			gui.pixel(i+2, 220-bit.quotent(enc1,0x100), 0x7F00FF00)
			
			if enc1 >= 0x10000 then enc1 = 0xff00 end
			enc2 = (enc2+1) % 0x100
			if enc2 == 0 then enc3 = (enc3 + 0x11) % 0x100 end
			local rng = (memory2.ROM:byte( 0x00fd00+enc2)+enc3) % 0x100
			if rng < bit.quotent(enc1,0x100) then
				gui.line(i+2, max(220-rng, 190), i+2, 190, 0x7FFF0000)
				if count == -1 then
					count = i + 1
				end
				enc1 = 0
			else
				gui.line(i+2, max(220-rng, 190), i+2, 190, 0x7F0000FF)
			end
		end
		...
		if pack_index == 0xFF then
			local enc_veldt = (memory2.WRAM:byte( 0x1fa5)+1) % 0x40
			-- find a nonzero byte in the encountered formations bitarray.
			while memory2.WRAM:byte( 0x1ddd+enc_veldt) == 0 do
				enc_veldt = (enc_veldt+1) % 0x40
			end
			local enc5 = memory2.WRAM:byte( 0x1fa3)
			local enc4 = (memory2.WRAM:byte( 0x1fa2)+1) % 0x100
			if enc4 == 0 then enc5 = enc5 + 0x17 end
			local bit_index = (memory2.ROM:byte( 0x00fd00+enc4)+enc5) % 0x8
			local bitset = memory2.WRAM:byte( 0x1ddd+enc_veldt)
			-- find a nonzero bit, starting at a random position
			while bit.test(bitset,bit_index) == 0 do
			...
			local enc1 = (memory2.WRAM:byte( 0x1fa2)+1) % 0x100
			local enc2 = memory2.WRAM:byte( 0x1fa3)
			if enc1 == 0 then enc2 = enc2+0x17 end
			local a = (memory2.ROM:byte( 0x00fd00+enc1)+enc2) % 0x100

			if a > 0x50 then x = x+2 end
			if a > 0xa0 then x = x+2 end
			if a > 0xf0 then x = x+2 end
			formation = memory2.ROM:word( 0x0f4800+x)
		end
		...
		if mode == MODE_MAP then
			faceing = memory2.WRAM:byte( 0x00f6)
			xpos = memory2.WRAM:byte( 0x00c6)
			ypos = memory2.WRAM:byte( 0x00c8)
		else
			-- have to loop through all chars to find out who is the leader
			-- for id = 0, 0xF do
				-- local delta = 0x29*id
				-- if bit.test(memory2.ROM:byte( 0x0867+delta),7) then
					-- faceing = memory2.ROM:byte( 0x087f+delta)
					-- break
				-- end
			-- end

			faceing = memory2.WRAM:byte( 0x00b3) - 1
			xpos = memory2.WRAM:byte( 0x005c)
			ypos = memory2.WRAM:byte( 0x0060)
		end
		...
		-- sprint shoes?
		if mode == MODE_CAVE and bit.test(field_effects,5) then
			pixels = bit.quotent(pixels,2)
		end
		...
		if standing_still then
			if mode == MODE_MAP then -- it only takes 1 frame to start moving on the map
				if memory2.WRAM:byte( 0xB622) == 0 then -- unknown value that seems to work well.
					pixels = pixels+1
				end
			else
				local frame_rule = bit.quotent(memory2.WRAM:byte( 0x0014), 12)
				if memory2.WRAM:byte( 0x000d) == 0 then -- we didn't start moving yet.
					pixels = pixels + 4 - ((frame_rule + 0) % 4)
				else
					pixels = pixels + 3 - ((frame_rule + 3) % 4)
				end
			end
		end
		...
		be = 4*((memory2.WRAM:byte(0x021e)+pixels+battle_transition-1) % 0x3c) + 4
		
		-- Print the frame rule.
		gui.text(10,offset,"Frame rule: ".. (bit.quotent(be,4)%4))
		offset = offset+10

		gui.text(10,offset,string.format("be: %x vs %x, %x %x %x", memory2.WRAM:byte(0x00be), be,pixels,xpos,ypos))
		offset = offset+10

		if bit.test(formation, 15) then
			-- High bit of formation set: randomly add 0..3 to formation
			be = (be+1) % 0x100
			local a = memory2.ROM:byte(0x00fd00+be) % 4
			formation = bit.band(formation + a, 0x7FFF)
		end

		local info1 = memory2.ROM:byte( 0x0f5900+4*formation)
		local info2 = memory2.ROM:byte( 0x0f5900+4*formation+1)
		local info3 = memory2.ROM:byte( 0x0f5900+4*formation+2)
		local info4 = memory2.ROM:byte( 0x0f5900+4*formation+3)

		-- Build monster list
		local start = 0x0f6201+15*formation
		local present = memory2.ROM:byte(start)
		local which = {}
		local number_of_enemies = 0
		local enemy_slot_speed = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}
		local enemy_slot_name = {"", "", "", "", "", ""}
		for i = 1, 6 do
			if bit.test(present, i-1) then
				local id = memory2.ROM:byte(start+i)
				if which[id] then which[id] = which[id]+1
				else which[id] = 1 end
				number_of_enemies = number_of_enemies + 1
				
				-- Grab the enemy speed
				enemy_slot_speed[i] = memory2.ROM:byte( 0x0f0001+0x20*id)
		...
		for i = 0, 3 do
			allowed[i] = not bit.test(info1, 4 + i) -- if checked, then disable corresponding encounter
			if allowed[i] then
				allowed_number = allowed_number + 1
			end
		end
		if bit.test(status_effects,1) then
		...
		for i = 0, 3 do if allowed[3-i] then
			sum = sum + memory2.ROM:byte( 0x025261+i)+1
		end end
		...
		be = (be+1) % 0x100
		local rng = bit.band(bit.lrshift(memory2.ROM:byte( 0x00fd00+be)*sum, 8), 0xFF)
		...
		if bit.test(info4, 2) then
			gui.text(10, offset, "Preemptive disabled")
			offset = offset+10
		elseif chosen == 0 or chosen == 3 then -- front or side, preemptive possible
			...
			if bit.test(status_effects,0) then -- Gale hairpin equipped
				rate = rate * 2
			end

			-- call the rng
			be = (be+1) % 0x100
			local rng = memory2.ROM:byte( 0x00fd00+be)
		...
		-- Finally, describe the result
		if preemptive then
			gui.text(10, offset, "Pre-emptive battle")
			offset = offset+10
		end
		...
		for entity = 9, 0, -1 do 
			...
			be = (be+1) % 0x100
			local rng = bit.band(bit.lrshift(memory2.ROM:byte( 0x00fd00+be)*entity_remaining, 8), 0xFF)
			
			-- take the rnd-th set bit starting 0
			local specific_incrementor
			for b = 0, 9 do
				if bit.test(entity_bit, b) then
					rng = rng - 1
				end
				if rng < 0 then
					specific_incrementor = b * 8
					entity_bit = bit.bxor(entity_bit, bit.lshift(0x1, b))
					break
				end
			end
			...
			if entity < 4 then -- character
				...
				elseif chosen == 0 then -- Front attack
					...
					be = (be+1) % 0x100
					local random_speed = bit.band(bit.lrshift(memory2.ROM:byte( 0x00fd00+be)*speed, 8), 0xFF)
		...
		for cs = 1, 4 do if slot_speed[cs] ~= 0xFF then -- character slot is filled
				gui.text(10, offset, string.format("ATB %s: %d",slot_name[cs], atb_bar[cs-1]))
				offset = offset+10
		end	end
		
		for ms = 1, 6 do if enemy_slot_speed[ms] ~= 0xFF then -- monster slot is filled
				gui.text(10, offset, string.format("ATB %s: %d",enemy_slot_name[ms], atb_bar[ms+3]))
				offset = offset+10
		end	end
	end
end
...
printall = function()
	...
	if memory2.WRAM:byte( 0x2000) < 13 then
		mode = MODE_BATTLE
	elseif bit.band(memory2.WRAM:word( 0x1f64), 0x01FF) <= 0x0002 then
		mode = MODE_MAP
	elseif bit.band(memory2.WRAM:word( 0x1f64), 0x01FF) >= 0x0003 and bit.band(memory2.WRAM:word( 0x1f64), 0x01FF) <= 0x019E then
		mode = MODE_CAVE
	end
	if memory2.WRAM:word( 0x1501) == 0x1387 and memory2.WRAM:byte( 0x1503) == 0xC3 then
		mode = MODE_MENU
...
callback.register("paint", printall)