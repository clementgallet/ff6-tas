...
function display_battle()
	local sel_side, sel_i = memory2.WRAM:byte( 0x7ace), memory2.WRAM:byte( 0x7acf)
	local sel_enemy, sel_party = memory2.WRAM:byte( 0x7b7e),memory2.WRAM:byte( 0x7b7d)
	local sel_multi = memory2.WRAM:byte( 0x7b7f)
	...
	for i = 0, 5 do
		...
		local x = memory2.WRAM:byte( 0x80c2+1+j)
		...
		-- I can't be bothered
		local xsize = memory2.WRAM:byte( 0x807a+1+j)
		if xsize == 0 then xsize = 0x20 end
		local y = memory2.WRAM:byte( 0x80ce+1+j)
		...
		local menu_pos = memory2.WRAM:byte( 0x890f+active_slot)
	...
	if memory2.WRAM:byte( 0x7bd4) == 0x24 then
		local x = memory2.WRAM:byte( 0x892f)
		local y = memory2.WRAM:byte( 0x8933)
		local scroll = memory2.WRAM:byte( 0x892b)
...
display_treasures = function()
	...
	local sx     = memory2.WRAM:word( 0x8297)
	local sy     = memory2.WRAM:word( 0x8299)
...
function display_encounters()
	...
	if mode == MODE_MAP then
		...
		local at_22 = memory2.ROM:byte( 0x00c28f+terrain) -- lave tall, men hva betyr de?
		local a = memory2.ROM:byte( 0x00c297+terrain) -- denne arrayen fÃ¸lger rett etter. samme stÃ¸rrelsesorden.
		...
		encounter_rate = memory2.ROM:word( 0x00c29f+x)
	else
		local area = memory2.WRAM:word( 0x0082)
		...
		encounter_rate = memory2.ROM:word( 0x00c2bf+x)
	...
	if encounter_rate ~= 0 then
		...
		if standing_still then
			if mode == MODE_MAP then -- it only takes 1 frame to start moving on the map
				if memory2.WRAM:byte( 0xB652) == 0 then -- unknown value that seems to work well.
					pixels = pixels+1
				end
			else
		...
		for i = 0, 3 do if allowed[3-i] then
			sum = sum + memory2.ROM:byte( 0x025279+i)+1
		end end
